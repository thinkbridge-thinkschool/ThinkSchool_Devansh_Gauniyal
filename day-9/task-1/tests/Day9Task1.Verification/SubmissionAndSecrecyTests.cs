...
{
    [Fact]
    public void Submission_file_exists_with_all_four_required_headings_and_the_anomaly_table()
    {
        ...
        // The anomaly -> lowest-preventing-level table the exercise asks for.
        Assert.Contains("READ COMMITTED", text);
        Assert.Contains("REPEATABLE READ", text);
        Assert.Contains("SERIALIZABLE", text);
    }

    private static readonly Regex[] SecretPatterns =
    [
        new(@"MSSQL_SA_PASSWORD\s*=\s*(?!\$)[^\s""']{4,}", RegexOptions.IgnoreCase), // literal value, not a $VAR reference
        new(@"SQLCMDPASSWORD\s*=\s*(?!\$)[^\s""']{4,}", RegexOptions.IgnoreCase),
        ...
        new(@"Aa1![A-Za-z0-9]{15,}"), // this orchestrator's own generated-password shape
    ];
    ...
    public static IEnumerable<object[]> AllTrackedFiles()
    {
        ...
        foreach (var file in Directory.EnumerateFiles(root, "*", SearchOption.AllDirectories))
        {
            ...
            // This test project's own source is hand-authored, never
            // generated from a real run, and its secret-detection regex
            // literals otherwise match themselves as text. Real captured
            // secrets can only ever land in sql/, scripts/, output/,
            // README.md or submission.md, so those are the files worth
            // scanning.
    ...
    [Fact]
    public void No_mdf_or_ldf_or_temp_orchestrator_artefacts_are_tracked()
    {
        var root = Paths.FindTaskRoot();
        var offenders = Directory.EnumerateFiles(root, "*", SearchOption.AllDirectories)
            ...
            .Where(f => f.EndsWith(".mdf", StringComparison.OrdinalIgnoreCase) ||
                        f.EndsWith(".ldf", StringComparison.OrdinalIgnoreCase) ||
                        Path.GetFileName(f).StartsWith("fifo", StringComparison.OrdinalIgnoreCase) ||
                        Path.GetFileName(f).StartsWith("batch_", StringComparison.OrdinalIgnoreCase))
            .ToList();
