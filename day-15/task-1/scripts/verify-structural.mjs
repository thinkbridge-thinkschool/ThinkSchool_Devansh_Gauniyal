...
// instead to files that are actually a component (`@Component(`) or an interceptor
// (`: HttpInterceptorFn`), which is exactly what the requirement names.
const constructorOffenders = nonSpecTsFiles.filter((f) => {
  const content = readFileSync(f, 'utf-8');
  const isComponentOrInterceptor = content.includes('@Component(') || content.includes(': HttpInterceptorFn');
  return isComponentOrInterceptor && content.includes('constructor(');
});
check(
  'no component or interceptor uses constructor-parameter injection',
