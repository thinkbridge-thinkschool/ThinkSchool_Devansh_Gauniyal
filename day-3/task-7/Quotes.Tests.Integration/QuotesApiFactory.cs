...
public sealed class QuotesApiFactory : WebApplicationFactory<Program>
{
    ...
    {
        DatabaseName = $"quotes_test_{Guid.NewGuid():N}";
    ...
    public async Task<int> CountQuotesAsync()
    {
        using var scope = Services.CreateScope();
        var database = scope.ServiceProvider.GetRequiredService<QuotesDbContext>();
        return await database.Quotes.CountAsync();
    }

    public async Task<Quote?> FindQuoteAsync(int id)
    {
        using var scope = Services.CreateScope();
        var database = scope.ServiceProvider.GetRequiredService<QuotesDbContext>();
        return await database.Quotes.AsNoTracking().SingleOrDefaultAsync(quote => quote.Id == id);
    }

    public async Task<bool> HasCaseInsensitiveTextMatchAsync(string text)
    {
        using var scope = Services.CreateScope();
        var database = scope.ServiceProvider.GetRequiredService<QuotesDbContext>();
        return await database.Quotes.AnyAsync(quote => quote.Text == text);
    }
}
