...
public sealed class DisputeInvoiceUseCase(
    ...
    TimeProvider clock)
{
    public async Task ExecuteAsync(InvoiceId invoiceId, string reason, CancellationToken cancellationToken)
    {
        ...
        // Day 30 - the other trigger DESIGN.md names for a supplier notification
        // ("on approval/dispute"). No outbox event here - only InvoiceApproved is
        // named as an integration event for a downstream (Financing) consumer;
        // a dispute has no such consumer today.
        await supplierNotifier.NotifyInvoiceDisputedAsync(invoice.SupplierId, invoice.InvoiceNumber, reason, cancellationToken);
    }
}
