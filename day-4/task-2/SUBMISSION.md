...
GET /api/quotes has no auth check at all — a missing test led me straight to a real security gap, not just a coverage gap. Also learned that config-validation guard clauses need direct unit tests, since HTTP-level tests can never reach them.

## 10. What would break this?

The public-read test locks in current behavior — if that's a bug and not a design choice, it breaks the moment someone fixes it. Also, branch coverage isn't provable across merged reports the way line coverage is — only line coverage is rigorous here.
