# Capstone infrastructure (Day 23 Task 1 — Bicep IaC; Day 24 Task 1 — Deployment Stacks + azd; Day 25 Task 1 — identity end-to-end; Day 27 Task 1 — network isolation + API hardening)
...
- **No private networking (closed for SQL on Day 27 - see that section below;
  still open for Key Vault and Service Bus).** Service Bus's Standard SKU
  cannot have network-level restriction of any kind (confirmed live on Day 27
  - private endpoints AND network rule sets are both Premium-only), so it
  remains reachable from the public internet, gated only by identity-based
  auth. Key Vault's private endpoint is unbuilt - a named, not-yet-built gap,
  same as before Day 27.
...
## Network isolation and API hardening (Day 27)

Full detail (the threat model, the ZAP scan and what was fixed, the exact
before/after connectivity evidence) is in `capstone/THREAT-MODEL.md` and
`capstone/submission-day-27-task-1.md`. This section covers what changed here,
in `infra/`, and why.

### SQL: a real private endpoint

`modules/network.bicep` (new) creates `vnet-capstone-{env}` with two subnets -
`snet-appservice-integration` (delegated to `Microsoft.Web/serverFarms`, used
by both Web Apps' regional VNet Integration) and `snet-private-endpoints`
(network policies disabled, holding SQL's private endpoint NIC) - plus the
`privatelink.database.windows.net` private DNS zone, linked to the VNet, so
the SQL server's normal hostname resolves to its new private IP for anything
inside the VNet. `modules/sql.bicep` sets `publicNetworkAccess: 'Disabled'`
and drops the `AllowAllAzureServices` firewall rule entirely (it has nothing
left to gate). Both Web Apps (`modules/api.bicep`, `modules/worker.bicep`) get
`virtualNetworkSubnetId` pointed at the integration subnet and
`vnetRouteAllEnabled: true`, without which there is no route to the private IP
at all.

### Service Bus: why it did NOT get the same treatment

The task asked for both SQL and Service Bus behind private endpoints. Two
real, live-confirmed Azure restrictions ruled that out for Service Bus on
this namespace's Standard SKU:

1. A private endpoint against the real `sb-capstone-dev-tnjxju` namespace was
   rejected outright: `az network private-endpoint create ...` returned
   `PrivateEndpointInvalidSku: "Private endpoint connections are only
   supported on Premium Service Bus namespaces."`
2. The fallback attempted next - a `Microsoft.ServiceBus/namespaces/networkRuleSets`
   resource (IP/VNet rules, not a private IP, but a real access filter) - was
   *also* rejected on the same namespace: `InvalidSkuForNetworkRuleSet: "Sku
   'Standard' does not support network rule set."`

Service Bus's entire network-restriction feature set (private endpoints,
VNet rules, IP firewall rules) is Premium-only - there is no cheaper partial
option. Premium's smallest SKU prices at $0.9275/hour (confirmed live against
the Azure Retail Prices API for Central India, 2026-09-11) - roughly 69x
Standard's cost, and left running per this task's "leave the dev stack
running" instruction would burn through most of this subscription's
remaining credit in about ten days. Given that real trade-off (raised with,
and decided by, the person running this session), Service Bus stays on
Standard, `publicNetworkAccess: 'Enabled'`, with identity-based auth
(`disableLocalAuth: true`, Day 25) as its only real control - a genuine, not
silently dropped, gap. See `capstone/THREAT-MODEL.md`'s Information
disclosure section for the full consequence of this.

### Proving the block is real, not just a property flip

A TCP-level check (`nc -zv sql-capstone-dev-2fqdji.database.windows.net
1433`) still succeeds even with public access disabled - Azure SQL's gateway
is shared infrastructure that accepts the TCP handshake regardless, then
denies the connection at the TDS pre-login layer for this specific server. A
real login attempt from outside Azure (a throwaway `Microsoft.Data.SqlClient`
console app, wrong credentials, run from this session's own machine) gets the
real, decisive error:
...
The same server, called from inside the VNet (the deployed app's own
`GET /demo/db-ping`, now behind auth - see below), returns a real result:
`{"pinged":true,"result":1,"atUtc":"..."}`. Both captured live this session -
see the submission file for the exact commands and full output.

### A real Deployment Stack limitation hit this session

Removing the `AllowAllAzureServices` firewall rule from `modules/sql.bicep`
made the next `az stack sub create` try to delete it (this stack's
`actionOnUnmanage: deleteAll` - see the Day 24 section above). That delete
failed: `DenyAssignmentAuthorizationFailed` - the stack's own `denyDelete`
deny-assignment blocked the stack itself from deleting a resource it was
trying to unmanage, a documented Deployment Stacks limitation
(https://aka.ms/DeploymentStacksKnownLimitations), not a bug in this
template. Fixed by re-running `az stack sub create` once with
`--deny-settings-mode none` (clearing the stuck resource), then once more
with `--deny-settings-mode denyDelete` restored - the stack ends in exactly
the protected posture it started in, with the stale resource actually gone.

### API hardening: authentication, versioning, input limits

The API surface through Day 26 was two demo endpoints proving DI wiring
resolves - nothing shaped like a real API to harden. Day 27 adds a real,
versioned `/v1` surface (`POST /v1/purchase-orders`, `POST /v1/invoices`,
`GET /v1/invoices`) alongside the existing demo endpoints, all detailed in
`host/Capstone.Web/Program.cs` and the submission file:

- **Authentication**: Entra ID JWT bearer, via a new app registration
  (`capstone-api-dev`, app id supplied as `CAPSTONE_API_AAD_APP_ID` -
  `modules/api.bicep` sets `Auth__TenantId` from `subscription().tenantId`
  and `Auth__ApiAppId` from that parameter as app settings - both are
  identifiers, not secrets). Local `dotnet run` has neither set by default,
  so auth is not enforced locally (README.md's existing unauthenticated demo
  instructions keep working); every deployed environment always sets both,
  so the deployed API always enforces it. `/`, the one true health check,
  stays anonymous.
- **Versioning**: the `/v1` route group - a real place to put a `/v2` later
  without breaking existing callers.
- **Input limits**: a 16 KB request body cap (endpoint filter), bounded
  string lengths (invoice number, item reference), bounded quantities/prices,
  page size clamped to 100, and a 100-requests-per-minute-per-IP rate limiter
  (`Microsoft.AspNetCore.RateLimiting`, built into ASP.NET Core) applied
  globally.

### ZAP baseline scan

Run with `docker run ghcr.io/zaproxy/zaproxy:stable zap-baseline.py -t
https://capstone-dev-api-f7nsoj.azurewebsites.net`, before and after fixing
what it found. Full report, findings, and fixes are in the submission file -
this file only points there to avoid duplicating a result that belongs with
its evidence.
