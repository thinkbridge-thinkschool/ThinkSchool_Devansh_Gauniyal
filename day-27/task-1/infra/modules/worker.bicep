...
@description('Day 27: resource id of the App Service VNet Integration subnet, from modules/network.bicep - same subnet the API uses (modules/api.bicep), since both apps share one plan and both need the same private route to SQL.')
...
resource webApp 'Microsoft.Web/sites@2026-07-15' = {
  ...
  properties: {
    ...
    httpsOnly: true
    // Day 27: same reasoning as modules/api.bicep - no per-instance state to pin
    // sessions to.
    clientAffinityEnabled: false
    keyVaultReferenceIdentity: apiIdentityResourceId
    // Day 27: same reasoning as modules/api.bicep - required now that SQL is
