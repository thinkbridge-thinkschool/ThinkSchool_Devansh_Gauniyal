...
resource sqlServer 'Microsoft.Sql/servers@2025-01-01' = {
  ...
  properties: {
    ...
    // Day 27: was 'Enabled' through Day 26, with the AllowAllAzureServices
    // firewall rule below as the only gate - any Azure-hosted resource in the
    // world could reach this server's public endpoint (identity-based auth,
    // Day 25, meant a stolen credential couldn't authenticate, but the network
    // path itself was open to anyone who had one, or who could brute-force
    // login attempts against it). Disabled now that modules/network.bicep gives
    // the API and worker a private path in - see capstone/THREAT-MODEL.md,
    // Information disclosure, and ../README.md's Day 27 section for the real
    // before/after connectivity proof.
...
// Day 27: the AllowAllAzureServices firewall rule (0.0.0.0-0.0.0.0, "let any
// Azure-hosted resource in the world through") that lived here through Day 26 is
// removed, not left in place-but-dormant - with publicNetworkAccess now
// 'Disabled' above, there is no public endpoint left for a firewall rule to gate
// at all, and leaving a stale allow-all rule in the template would misdescribe
// the actual network model to the next reader. Reachability is now entirely
// through modules/network.bicep's private endpoint.
