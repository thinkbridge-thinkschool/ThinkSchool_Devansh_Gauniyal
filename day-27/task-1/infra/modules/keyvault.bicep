...
@secure()
@description('Day 26: the Application Insights connection string (from modules/monitoring.bicep). Marked secure and stored here, not as a plaintext app setting, per this task\'s "an App Insights connection string contains a key and must NOT be committed" rule - even though actual ingestion authentication uses the managed identity\'s Entra ID token, not this string, once DisableLocalAuth is set (see modules/monitoring.bicep).')
...
resource appInsightsConnectionStringSecret 'Microsoft.KeyVault/vaults/secrets@2026-05-15' = {
  parent: vault
