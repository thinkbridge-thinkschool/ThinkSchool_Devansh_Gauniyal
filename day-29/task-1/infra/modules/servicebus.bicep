...
resource namespace 'Microsoft.ServiceBus/namespaces@2026-01-01' = {
  ...
  properties: {
    ...
    // Day 27: stays 'Enabled' - this namespace's Standard SKU has NO network-level
    // access control at all, confirmed live twice this session against this real
    // namespace: a private endpoint was rejected outright
    // (PrivateEndpointInvalidSku), and the fallback attempted here - a
    // Microsoft.ServiceBus/namespaces/networkRuleSets resource restricting the
    // namespace to one VNet subnet - was ALSO rejected
    // (InvalidSkuForNetworkRuleSet: "Sku 'Standard' does not support network rule
    // set"). IP firewall rules, VNet rules, and private endpoints are all
    // Premium-only on Service Bus, not just Private Link specifically - this
    // project's own README.md already named Premium's ~69x cost as unjustified
    // for a namespace nothing calls yet (Day 23), and upgrading it just to gain a
    // network control this dev environment doesn't otherwise need was ruled out
    // for the same reason - see capstone/THREAT-MODEL.md, Information
    // disclosure, for what is left open as a result: this namespace is reachable
    // from the public internet, gated only by identity-based auth
    // (disableLocalAuth below, Day 25). Setting this to 'Disabled' would remove
    // the public listener entirely with no VNet path to replace it (no network
    // rule set can exist on this SKU to allow one back in), so it stays
    // 'Enabled' rather than being set to a value that would just break the app.
