...
@description('Day 27: the API\'s own Entra ID app registration id (see submission-day-27-task-1.md) - not a secret, but sourced from the environment at deploy time like every other identifier in this file, via CAPSTONE_API_AAD_APP_ID.')
...
// Day 27: the VNet, subnets, and SQL's private endpoint - see
// modules/network.bicep's header comment and modules/servicebus.bicep's own Day
// 27 comment for why Service Bus gets neither a private endpoint nor a network
// rule set (both are Premium-SKU-only, confirmed live against this real Standard
// namespace).
module network 'modules/network.bicep' = {
  name: 'capstone-network-${environmentName}'
  scope: rg
  params: {
    environmentName: environmentName
    location: location
    sqlServerResourceId: sql.outputs.serverResourceId
    tags: tags
  }
}
