...
- **Managed-identity proof — real, live, captured to `output/`:**
  ...
  - **Immediately after the assignment, still `403`** — Azure.Identity's managed-identity credential had already cached a token from before the role existed. `az functionapp restart` cleared it.
  - **After the restart** (`output/function-call-after-role-assignment.txt`): `GET .../api/quotes` on the Function → `200 OK`, with response headers `X-Managed-Identity-Audience-Verified: true`, `X-Managed-Identity-Role-Verified: Api.Access`, `X-Managed-Identity-Authorized-Endpoint: /api/protected`, and the real quotes payload — proof a real token's audience and role were verified, the token's value never printed anywhere, only these derived booleans and claim names.
  - **The negative proof** (`output/get-protected-401-live.txt`), re-captured fresh: `GET https://quotesapi-devansh-d17t1.azurewebsites.net/api/protected` with no Authorization header at all → `401`, `WWW-Authenticate: Bearer`, no body — the positive result above means something, since the same endpoint genuinely refuses an absent token.
  - The endpoint proven throughout is `GET /api/protected` — the genuinely authorized route — never the anonymous `/api/quotes` list.
- **A second real mistake, caught and fixed on live infrastructure:** `az functionapp create`'s own defaults silently added two things this task forbids without approval — a plaintext storage connection string in `AzureWebJobsStorage`, and a full Application Insights resource. Both were removed before anything else proceeded: the connection string was replaced with `AzureWebJobsStorage__accountName` plus three data-plane RBAC roles granted to the identity (`Storage Blob Data Owner`, `Storage Queue Data Contributor`, `Storage Table Data Contributor`); the Application Insights resource was deleted outright. One caveat disclosed rather than hidden: `WEBSITE_CONTENTAZUREFILECONNECTIONSTRING`, Azure's own platform-internal setting for the Consumption plan's deployment/content-share mechanism, still holds a connection string — it is not read by the Function's own code, and converting it is a separate, more involved Azure feature not completed here.
- **Secret scan — confirmed clean, commands shown, including the new live-capture output files:**
  ```bash
  ...
  Neither the real tenant GUID nor the real client GUID appears in any tracked file, in any captured output, or in this document. Both appear only as `YOUR_TENANT_ID` / `YOUR_CLIENT_ID` placeholders in tracked files; the real values were supplied only as process arguments and Azure application settings at deployment time. No client secret, connection string, key, or token appears in the repository or either app's settings, beyond the disclosed platform-internal content-share exception above.
- **States exercised:** loading, error, and empty are already exercised by the 107 carried Angular tests (unchanged) plus the app's own carried empty/error/loading UI states. The 401-without-token state is now proven twice: by the copied `QuotesApi.Tests` suite (20/20 passing) and, for real, by the live capture above against the actual deployed API.
...
- The deployed app's *general* traffic (login, browsing, creating quotes) points at the real API directly, not the Function — the Function is reserved for the managed-identity proof alone. Pointing the whole SPA at the Function's single implemented route was a real bug caught by actually using the live site (login failed with a 404 from the Function, not a 401 from the API); see `verification-log.md` for the full diagnosis and fix.
