...
builder.Services.AddOpenTelemetry()
    ...
    .WithTracing(tracing => tracing
        ...
        // traces reach a locally running Jaeger instance. Harmless to leave in when
        // deployed: with nothing listening at that address inside the container, it
        // just fails to connect silently, same as verified in Task 2.
        .AddOtlpExporter())
    // Day 5 Task 5: the piece that was missing. UseAzureMonitor() reads
    // APPLICATIONINSIGHTS_CONNECTION_STRING from configuration/environment
    // automatically when no connection string is set explicitly -- that variable is
    // already present on the deployed Container App (confirmed in Step 1), it was
    // simply never read by any code until now.
    .UseAzureMonitor();
